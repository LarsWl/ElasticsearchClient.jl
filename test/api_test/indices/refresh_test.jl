...
@testset "Testing create method" begin
  ...
  apply(client_patch) do
    @test Elasticsearch.Indices.refresh(client, index=test_index) isa Elasticsearch.API.Response
    @test Elasticsearch.Indices.refresh(client) isa Elasticsearch.API.Response
  end
end
