...
@testset "Testing create method" begin
  ...
  apply(client_patch) do
    @test ElasticsearchClient.Indices.refresh(client, index=test_index) isa ElasticsearchClient.API.Response
    @test ElasticsearchClient.Indices.refresh(client) isa ElasticsearchClient.API.Response
  end
end
